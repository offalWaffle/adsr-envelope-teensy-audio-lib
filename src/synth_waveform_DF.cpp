...
void AudioSynthWaveform_DF::update(void)
{
	updateMidiInput();
...
void AudioSynthWaveform_DF::updateMidiInput()
{
	...
	{
		frequency(pow(2,(midiVoiceData.noteNumber - 69)/12.0f) * 440.0f);
        midiVoiceData++;
	}
...
void AudioSynthWaveformModulated_DF::update(void)
{
...
void AudioSynthWaveformModulated_DF::updateMidiInput()
{
	...
	midiData = receiveReadOnly(3);
	if(!midiData) return;

	int8_t * noteNumber = (int8_t *) midiData->data;
